...
(function() {
  ...
  Spine.Route = (function(_super) {
    ...
    Route.add = function(path, callback, options) {
      var key, route, value, _results;
      if (options == null) {
        options = {};
      }
      if (typeof path === 'object' && !(path instanceof RegExp)) {
        ...
        for (key in path) {
          value = path[key];
          _results.push(this.add(key, value, options));
        }
        return _results;
      } else {
        route = new this(path, callback);
        if (options.trigger) {
          route.match(this.path);
        }
  ...
  Spine.Controller.include({
    route: function(path, callback, options) {
      return Spine.Route.add(path, this.proxy(callback), options);
    },
    routes: function(routes, options) {
      ...
      for (key in routes) {
        value = routes[key];
        _results.push(this.route(key, value, options));
      }
      return _results;
    },
    navigate: function() {
